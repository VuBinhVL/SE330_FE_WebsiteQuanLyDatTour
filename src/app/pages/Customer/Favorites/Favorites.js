...
export default function Favorites() {  const navigate = useNavigate();
    ...
    fetchGet(
      ...
      (res) => {
        ...
        if (favoriteData.length > 0) {
          const promises = favoriteData.map(
            (fav) =>
              new Promise((resolve) => {
                fetchGet(
                  ...
                  (routeRes) => {
                    ...
                    fetchGet(
                      ...
                      (toursRes) => {
                        ...
                        const routeTours = allTours.filter(
                          (tour) => tour.tourRouteId === fav.tourRouteId
                        );

                        // Lấy giá thấp nhất và các ngày khởi hành
                        const prices = routeTours
                          .map((t) => t.price)
                          .filter((p) => p);
                        const dates = routeTours
                          .map((t) => t.depatureDate)
                          .filter((d) => d);
  ...
  const removeFavorite = (favoriteId, tourName) => {
    if (
      window.confirm(
        `Bạn có chắc muốn xóa "${tourName}" khỏi danh sách yêu thích?`
      )
    ) {
      fetchDelete(
  ...
  const handleBookNow = (tourRouteId) => {
    ...
    fetchGet(
      ...
      (res) => {
        ...
        const routeTour = allTours.find(
          (tour) => tour.tourRouteId === tourRouteId
        );
        if (routeTour) {
    ...
    <div className="favorites-container">
      ...
      <main className="favorites-main">
        <div className="favorites-header">
          <h2 className="favorites-title">YÊU THÍCH ĐÃ LƯU</h2>
        </div>
        ...
        <div className="favorites-content">
          <div className="favorites-top-bar">
            <div className="favorites-stats">
              <div>
                Tổng cộng:{" "}
                <span className="favorites-count">{favorites.length}</span> tour
                yêu thích
              </div>
            </div>
          </div>
                  ...
                  <div className="favorites-tour-card" key={tour.id}>
                    <div className="favorites-tour-img">
                      ...
                      <button
                        ...
                        onClick={() =>
                          removeFavorite(tour.favoriteId, tour.name)
                        }
                      >
                        <span
                          role="img"
